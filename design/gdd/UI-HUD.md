...
2. ~~**보스방 레이아웃**~~ — **해결됨(2026-08-23)**: GDD-vs-구현 자체 감사로 발견 — Tuning Knobs 표가 이미 "보스 HP 바 크기: 대형 (일반 2배)"로 값을 확정해뒀는데 `battle_screen.gd`엔 `is_boss` 참조가 아예 없어 보스도 일반 적과 동일한 10px HP 바였음. `is_boss=true`일 때 HP 바 높이 20px(일반 2배)로 분기해 이미 확정된 튜닝값 그대로 배선. 위치/레이아웃 재배치는 하지 않음(카드 자체는 다른 유닛과 동일 그리드에 그대로 유지 — Tuning Knobs가 "크기"만 명시했고 "위치"는 별도 요구 없음).
   - 담당: ux-designer | 해결 시점: ~~`/ux-design`~~ → 해결됨 (위 참조)
