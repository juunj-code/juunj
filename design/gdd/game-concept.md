...
3. **동료 수집 및 영구 해금 시스템** — 런 종료 후 영구 로스터 추가 (2026-07-28 리비전 — 교차 리뷰에서 발견: "런 내 합류"는 18개 GDD 어디에도 구현되지 않아 MVP에서 명시적으로 컷, 아래 "Explicitly NOT in MVP" 참조. 발견은 즉시 파티 합류가 아니라 도감 등록으로 처리 — `#3 동료 해금`/`#15 파티 구성` 참조)
...
- **Difficulty scaling**: ~~해금된 동료 수에 따라 적 강도 자동 조절~~ — **MVP에서 컷 (2026-07-28 재검토에서 발견 및 정리)**: 18개 GDD 어디에도 구현되지 않았고("런 내 합류"와 동일한 유형의 orphaned promise), MVP 경제 설계상 런 간 지속되는 파워(장비는 런마다 초기화, 동료는 선택지만 늘림— 스탯 파워 아님)가 없어 스케일링이 상쇄할 대상 자체가 없다는 점도 확인됨. `#11 적-데이터`는 층별 고정 스탯 범위로 확정. Full Vision에서 재검토 시 실제 필요성부터 다시 판단할 것.
...
- 장비 시스템 (기본 스탯만)
- 보상형 광고, 앱 버전
- **런 내 동료 합류** (2026-07-28 추가 — 교차 리뷰에서 발견된 컷 확정): 히든방에서 발견한 동료가 그 즉시 파티에 합류해 같은 런에서 사용 가능해지는 것. MVP는 "발견 = 도감 영구 등록"까지만 처리하고, 그 런에서는 사용 불가 — 다음 런 파티 구성(`#15`)부터 선택 가능. Full Vision에서 재검토.
