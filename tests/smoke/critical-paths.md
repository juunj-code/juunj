...
**Run via**: `/smoke-check` (which reads this file), or manually.
...
5. Save game completes without error (once #17 로컬-세이브 is implemented)
6. Load game restores correct state (once #17 로컬-세이브 is implemented)
...
9. `godot --headless --path . -s res://addons/gut/gut_cmdln.gd -gexit` passes (0 failures)
