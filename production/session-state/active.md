...
**완료된 시스템 요약**: #6 전투 공식·#10 동료 데이터·#11 적 데이터(+DataValidator 빌드타임 검증기)·#7 적 AI·#12 상태이상·#19 씬 관리(순수 로직)·#13 런 상태 관리(RunManager, inventory 포함)·#15 파티 구성·#4 장비·#17 로컬 세이브(동기 코어)·#14 영구 진행. 전부 `src/core/`, 테스트는 `tests/unit/core/`.

**다음 후보 (언블록 상태)**:
- `#16 런 결과` — #13/#14 둘 다 완료, 언블록. UI 렌더링은 #20 소관, 이 시스템은 결과 데이터 조립(해금 목록·최고기록·성공여부)만 — #13/#14 패턴처럼 순수 로직 우선 추출 가능해 보임.
- `#2 랜덤 던전` — #13 완료, #19(Node) 미완성이지만 #13/#15가 그랬듯 defensive seam으로 던전 생성 순수 로직(방 배치·타입 분류)은 먼저 뽑아낼 수 있을 듯.
- `#9 히든 트리거` — #2·#14 둘 다 걸림(#2 아직, #14는 이제 완료). #2 먼저 필요.
- `#3 동료 해금` — #9에 의존, 연쇄 대기.
- `#1 턴제 전투` — 8개 hard dep 전부 이제 존재(#6,#7,#10,#11,#12,#13,#15,#4)! 다만 GDD 자체가 "L(4세션+)" 이고 실제 플레이어 입력 대기(`await action_selected`)가 핵심이라 UI 없이는 반쪽 구현 — 다음 큰 단계로 고려.
