...
**접근성 확인 + art-bible 6-9 작성 완료, 사용자 수면 중 자율 진행 (2026-08-04)** — 사용자가 "판단 필요하면 다음 세션에서 물어봐"라고 하고 자리를 비운 상태에서 진행. 코드 변경 없음(전부 문서 작업), 커밋 2개(`7294224`, `78d1b1a`).

- **접근성 Basic tier**: `accessibility-requirements.md`를 실제 구현 대비 재검증 — 9개 항목 중 5개가 "Not Started"에서 실제 코드 인용(`grep` 검증)으로 Satisfied 전환(상태이상 아이콘+텍스트 병행, 색맹 안전 스프라이트, 전체 버튼 44px 터치타겟, 1회성 300ms 플래시). Basic tier 자체도 proposed → confirmed. **여전히 열린 갭 2개**: 최소 텍스트 크기(명시적 px 미설정, Godot 기본 테마 그대로)와 텍스트 대비(커스텀 Theme 없음, 기본 테마라 아마 괜찮을 것으로 추정되나 실측 안 함) — 둘 다 코드로 고칠 문제가 아니라 실제 모바일 브라우저에서 눈으로 확인해야 하는 항목. **판단 필요**: 다음 실기기/모바일 플레이테스트 때 같이 확인할지, 아니면 별도로 스크린샷 찍어 대비 체크 도구로 검증할지 — 지금은 급한 게이트가 아니라서 보류.
- **art-bible 섹션 6-9**: 7(UI/HUD)·8(Asset Standards)·9(Reference)는 실제 코드/파일을 근거로 역문서화, 6(Environment)은 환경 아트 프로덕션이 아직 0건이라 텍스처 철학·무드 매트릭스 교차참조만 채우고 나머지(건축양식/프롭밀도/환경서사)는 명시적으로 보류(추측성 규정을 미리 만들지 않음).
  - **판단 필요 1 — 발견된 실제 갭**: 코드 감사 중 발견(고치지 않음) — 초상화 원본 PNG가 개당 4~5MB인데 실제 렌더는 40px 아이콘뿐(`party_select_screen.gd`), 상태이상 아이콘도 스펙(32×32, art-bible 5-1)과 실제 렌더(24×24, `battle_screen.gd`의 `_STATUS_ICON_SIZE`)가 다름. 지금 에셋 개수가 적어(총 11개) 웹 로딩엔 영향 없지만, 에셋이 늘어나기 전에 리사이즈/압축 정책을 정할지, 계속 미룰지는 사용자 판단.
  - **판단 필요 2** — 24×24 vs 32×32 불일치를 art-bible 스펙 쪽을 실제값(24×24)에 맞춰 고칠지, 아니면 코드를 스펙(32×32)에 맞춰 고칠지 — 둘 다 사소한 diff라 아무 쪽이나 빠르게 정리 가능, 그냥 어느 쪽이 맞는지 확인만 필요.
