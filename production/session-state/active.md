...
**4번 — 실측 필요 열린 질문 (같은 세션, 2026-08-23)** — 후보 2건 중 실제로 코드 감사/기존 데이터로 닫을 수 있는 것과, 진짜 사람 플레이테스트가 필요해 지어낼 수 없는 것을 구분해서 처리.

- **SP 초기값 OQ 닫음** (`전투-공식.md` OQ1): 위 커밋 `725a677` 참조 — 질문 전제 자체가 이미 실제 데이터(`cost_sp=0`)와 안 맞음, 코드 감사+기존 실측 데이터로 종결.
- **전투 페이싱 OQ("중앙값 턴 길이 ≤30초")는 그대로 열어둠** — `턴제-전투.md` 199~201행. 이건 실제 사람이 화면 보고 판단/입력하는 시간이 포함된 지표라 시뮬레이션이나 코드 감사로 대체 불가능(지어낼 수 없음) — GDD 자신도 "실제 플레이테스트 데이터 없이는 확정 불가"라고 명시. 근거 없이 억지로 닫지 않음.
- **다음**: 5번(자체 진단 — GDD vs 구현 대조로 새 갭 탐색)으로 진행.

**5번 — 자체 진단 (같은 세션, 2026-08-23)** — 아직 이 세션에서 안 훑은 GDD(`UI-HUD.md`)를 Detailed Rules/Tuning Knobs/Open Questions 전체 재독해로 대조.

- **실제 갭 발견+수정**: UI Requirement 표가 "보스 HP 바 | 별도 대형 HP 바 (is_boss=true 시)"를 명시하고 Tuning Knobs가 정확한 값("대형 (일반 2배)")까지 이미 확정해뒀는데, `battle_screen.gd`엔 `is_boss` 참조가 아예 없어 보스도 일반 적과 동일한 10px HP 바였음. `is_boss=true`일 때 20px(2배)로 분기 — 이미 확정된 튜닝값 그대로 배선(새 판단 불필요). `UI-HUD.md` Open Question #2("보스방 레이아웃")도 같이 종결.
- **검증**: 220/220 GUT 그대로(순수 시각). 창모드 스크린샷(`prototypes/boss-hpbar-verify/`, 검증 후 삭제) — `enemy_boss_01`("바람의 수호자") HP 바가 파티원 대비 눈에 띄게 두껍게 렌더링되는 것 확인. 커밋 `8d73c56`.
- **1~5번 전부 완료**. **다음**: 사용자 지시 대기.

**추가 자체 진단 — `장비.md` 대조 (같은 세션, 2026-08-23)** — "다음작업은?" 질문에 이어서 자율로, 이번 세션에 아직 안 훑은 GDD(#4 장비) 대조.

- **큰 실버그 발견+수정**: `Equipment.roll_and_apply_drop()`의 실제 호출부가 코드 전체에서 딱 하나뿐이었음 — `hidden_trigger.gd`(#9, 이미 해금된 히든방 재진입 케이스)뿐. 게임의 메인 장비 획득 루프인 "전투방 클리어 후 50% 확률 드롭"은 **한 번도 실행된 적이 없었음** — `RunManager.exit_combat_victory()`가 적만 지우고 바로 탐색으로 복귀, 드롭 롤 자체가 없었음. `equipment_test.gd` 자신의 헤더 주석이 "AC6/7/7b는 #2/#9가 아직 안 만든 호출부"라고 이미 실토하고 있었는데, #9 쪽만 만들어지고 #2(전투방 클리어) 쪽은 빠진 채 방치됐던 것.
- **수정**: `exit_combat_victory()`에 `current_room_data.type == "combat"`일 때만 `Equipment.roll_and_apply_drop(self, _run_rng)` 호출 추가(보스방은 그대로 제외, AC6). 런이 이미 소유한 시드 `_run_rng` 재사용(새 RNG 안 만듦).
- **검증**: 신규 GUT 테스트 2개(전투방 히트 시 드롭/보스방은 히트 rng여도 무드롭), 222/222 통과. 창모드 스크린샷(`prototypes/equipment-drop-verify/`, 검증 후 삭제) — 던전 화면에서 실제 아이템 드롭 팝업("철검 / 공격력 +3")이 정상 렌더링, "다음 방으로" 버튼이 팝업 중 비활성 확인(Core Rule 2). 커밋 `dc510b9`.
- **다음**: 사용자 지시 대기.

**실제 플레이 피드백 4건 (같은 세션, 2026-08-23)** — 사용자가 게임을 직접 띄워 플레이해보고 4가지 지적: (1) 베기 스킬이 안 눌림, (2) 적 2마리 이상일 때 카드 구분 안 됨, (3) HP 아래 동그라미(SP) 의미 불명, (4) BGM이 너무 단조롭고 캐릭터 아트가 3D에 가깝게 일관되게 재생성되어야 할 듯.

- **(1) 베기 비활성 — 정상 동작 확인, 버그 아님**: 아이라는 SP 0으로 시작, 베기는 SP 2 필요, SP는 자기 턴 종료마다 +1이라 3턴째부터 사용 가능(의도된 설계 — 적은 SP 소모 0이라 즉시 스킬 쓰지만 동료는 쌓아야 함, 이번 세션 앞부분에서 이미 코드로 확인한 비대칭과 동일 구조). 코드 변경 없음.
- **(2)+(3) 실제 버그 발견+수정**: 같은 종 적 2마리(예: 고블린 정찰병 x2)가 완전히 동일한 이름/초상화/HP라 카드 경계가 안 보였음 — 같은 id가 2번 이상이면 카드 이름에 순번 부여("고블린 정찰병 1"/"2"). SP 점(●●○○○)이 라벨 없이 표시되고 있었고 `UI-HUD.md`가 명시하지 않은 적 카드에도 표시되고 있었음 — "SP" 접두 라벨 추가 + 파티 카드에만 표시(적 카드는 제거). 창모드 스크린샷(`prototypes/enemy-overlap-verify/`, 검증 후 삭제)으로 수정 전/후 대조 확인, 222/222 GUT 그대로(순수 렌더링). 커밋 `c6aa9f9`.
- **(4) BGM 품질 + 캐릭터 아트 3D풍 재작업**: 둘 다 방향성 결정이 필요한 큰 스코프라 바로 실행 안 하고 사용자에게 옵션 제시 예정(진행 중).
- **다음**: 사용자 지시 대기.
